...
FOOT_JOINT_CANDIDATES = [7, 8, 10, 11]
...
def canonicalize_points(points_seq: np.ndarray, floor_y: float, height: float) -> np.ndarray:
    points = points_seq.copy().astype(np.float32)
    points[:, :, 1] *= -1.0
    points[:, :, 1] -= floor_y
    points[:, :, 1] += height * 0.1
    return points
...
def _safe_stat_range(values: np.ndarray) -> tuple[float, float, float]:
    arr = np.asarray(values, dtype=np.float64).reshape(-1)
    return float(arr.min()), float(arr.max()), float(arr.std())


def compute_sequence_metrics(verts_seq: np.ndarray, joints_seq: np.ndarray) -> dict[str, np.ndarray]:
    bbox_min = verts_seq.min(axis=1)
    bbox_max = verts_seq.max(axis=1)
    bbox_size = bbox_max - bbox_min
    bbox_center = (bbox_min + bbox_max) * 0.5
    bbox_height = bbox_size[:, 1]

    verts_delta_frame0 = np.linalg.norm((verts_seq - verts_seq[0]).reshape(len(verts_seq), -1), axis=1)
    joints_delta_frame0 = np.linalg.norm((joints_seq - joints_seq[0]).reshape(len(joints_seq), -1), axis=1)

    if len(verts_seq) > 1:
        verts_delta_prev = np.concatenate(
            [np.zeros(1, dtype=np.float32), np.linalg.norm(np.diff(verts_seq, axis=0).reshape(len(verts_seq) - 1, -1), axis=1)]
        )
    else:
        verts_delta_prev = np.zeros(1, dtype=np.float32)

    if len(joints_seq) > 1:
        joints_delta_prev = np.concatenate(
            [np.zeros(1, dtype=np.float32), np.linalg.norm(np.diff(joints_seq, axis=0).reshape(len(joints_seq) - 1, -1), axis=1)]
        )
    else:
        joints_delta_prev = np.zeros(1, dtype=np.float32)

    pelvis = joints_seq[:, 0, :]
    foot_indices = [i for i in FOOT_JOINT_CANDIDATES if i < joints_seq.shape[1]]
    if foot_indices:
    ...
    return {
        ...
        "verts_delta_frame0": verts_delta_frame0,
        "joints_delta_frame0": joints_delta_frame0,
        "verts_delta_prev": verts_delta_prev,
        "joints_delta_prev": joints_delta_prev,
        "pelvis": pelvis,
        "foot_min_y": foot_min_y,
    }


def debug_sequence_stats(
    label: str,
    verts_seq: np.ndarray,
    joints_seq: np.ndarray,
    focus_frames: Optional[List[int]] = None
):
    metrics = compute_sequence_metrics(verts_seq, joints_seq)
    focus_frames = focus_frames or []

    print(f"[DEBUG] ===== {label} SUMMARY =====")
    for key in ["bbox_height", "verts_delta_frame0", "joints_delta_frame0", "verts_delta_prev", "joints_delta_prev", "foot_min_y"]:
        mn, mx, std = _safe_stat_range(metrics[key])
        print(f"[DEBUG] {label} {key} min/max/std:", mn, mx, std)
    for axis, axis_idx in zip(["x", "y", "z"], [0, 1, 2]):
        mn, mx, std = _safe_stat_range(metrics["bbox_center"][:, axis_idx])
        print(f"[DEBUG] {label} bbox_center_{axis} min/max/std:", mn, mx, std)

    print(f"[DEBUG] {label} bbox_height spike frames:", find_spike_frames(metrics["bbox_height"]))
    print(f"[DEBUG] {label} bbox_center_x spike frames:", find_spike_frames(metrics["bbox_center"][:, 0]))
    print(f"[DEBUG] {label} bbox_center_y spike frames:", find_spike_frames(metrics["bbox_center"][:, 1]))
    print(f"[DEBUG] {label} bbox_center_z spike frames:", find_spike_frames(metrics["bbox_center"][:, 2]))
    print(f"[DEBUG] {label} verts_delta_prev spike frames:", find_spike_frames(metrics["verts_delta_prev"]))
    print(f"[DEBUG] {label} joints_delta_prev spike frames:", find_spike_frames(metrics["joints_delta_prev"]))
    print(f"[DEBUG] ===== {label} PER-FRAME =====")
    for i in range(len(verts_seq)):
        print(
            f"[DEBUG] {label} frame={i:04d} "
            f"bbox_min={_format_vec(metrics['bbox_min'][i])} "
            f"bbox_max={_format_vec(metrics['bbox_max'][i])} "
            f"bbox_size={_format_vec(metrics['bbox_size'][i])} "
            f"bbox_center={_format_vec(metrics['bbox_center'][i])} "
            f"bbox_height={float(metrics['bbox_height'][i]):.6f} "
            f"verts_df0={float(metrics['verts_delta_frame0'][i]):.6f} "
            f"joints_df0={float(metrics['joints_delta_frame0'][i]):.6f}"
        )
    print(f"[DEBUG] ===== {label} FOCUS FRAMES =====")
    for frame in focus_frames:
        if frame < 0 or frame >= len(verts_seq):
            print(f"[DEBUG] {label} frame={frame:04d} skipped (out of range)")
            continue
        print(
            f"[DEBUG] {label} frame={frame:04d} "
            f"bbox_min={_format_vec(metrics['bbox_min'][frame])} "
            f"bbox_max={_format_vec(metrics['bbox_max'][frame])} "
            f"bbox_size={_format_vec(metrics['bbox_size'][frame])} "
            f"bbox_center={_format_vec(metrics['bbox_center'][frame])} "
            f"pelvis={_format_vec(metrics['pelvis'][frame])} "
            f"foot_min_y={float(metrics['foot_min_y'][frame]):.6f} "
            f"verts_df0={float(metrics['verts_delta_frame0'][frame]):.6f} "
            f"joints_df0={float(metrics['joints_delta_frame0'][frame]):.6f} "
            f"verts_dprev={float(metrics['verts_delta_prev'][frame]):.6f} "
            f"joints_dprev={float(metrics['joints_delta_prev'][frame]):.6f}"
        )
    print(f"[DEBUG] ===== END {label} =====")
    return metrics


def debug_canonicalize_comparison(
    before_label: str,
    ...
    print(f"[DEBUG] ===== CANONICALIZE COMPARISON {before_label} -> {after_label} =====")
    height_delta = after_metrics["bbox_height"] - before_metrics["bbox_height"]
    center_delta = after_metrics["bbox_center"] - before_metrics["bbox_center"]
    for axis, axis_idx in zip(["x", "y", "z"], [0, 1, 2]):
        mn, mx, std = _safe_stat_range(center_delta[:, axis_idx])
        print(f"[DEBUG] canonicalize center_delta_{axis} min/max/std:", mn, mx, std)
    mn, mx, std = _safe_stat_range(height_delta)
    print("[DEBUG] canonicalize bbox_height_delta min/max/std:", mn, mx, std)
    print("[DEBUG] ===== CANONICALIZE COMPARISON FOCUS =====")
    for frame in focus_frames:
        if frame < 0 or frame >= len(height_delta):
            print(f"[DEBUG] canonicalize frame={frame:04d} skipped (out of range)")
            continue
        print(
            f"[DEBUG] canonicalize frame={frame:04d} "
            f"before_center={_format_vec(before_metrics['bbox_center'][frame])} "
            f"after_center={_format_vec(after_metrics['bbox_center'][frame])} "
            f"center_delta={_format_vec(center_delta[frame])} "
            f"before_height={float(before_metrics['bbox_height'][frame]):.6f} "
            f"after_height={float(after_metrics['bbox_height'][frame]):.6f} "
            f"height_delta={float(height_delta[frame]):.6f}"
        )
    print("[DEBUG] ===== END CANONICALIZE COMPARISON =====")
...
def main():
    ...
    reconstruct_metrics = debug_sequence_stats(
        "RECONSTRUCT_LOCAL",
        local_verts_seq,
        local_joints_seq,
    ...
    canonical_local_joints = canonicalize_points(local_joints_seq, floor_y, height)
    canonical_metrics = debug_sequence_stats(
        "CANONICAL_LOCAL",
        canonical_local_verts,
        canonical_local_joints,
        focus_frames=DEBUG_FOCUS_FRAMES,
    )
    debug_canonicalize_comparison(
        "RECONSTRUCT_LOCAL",
        reconstruct_metrics,
        "CANONICAL_LOCAL",
        canonical_metrics,
